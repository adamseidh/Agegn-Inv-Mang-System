...
import ProductItemTable from "./onEditProductListTable";
import ProductListModal from "./onEditProductListModal";

function AddProductOnEdit() {
  ...
  const addToCart = async (product, quantity) => {
    try {
      ...
      await axios.post(
        `${serverHost}/addSellsOnEdit`,
        {
          sells_id: saleId,
          product_id: product.id,
          quantity,
          unit_price: selling_price,
          total_price: parseFloat(selling_price) * parseFloat(quantity),
        },
        config
      );

      alert("Product successfully added to the current sale");
    } catch (error) {
          ...
          <div className="flex flex-col fixed z-20 bg-gray-100 py-4 w-full lg:w-1/5 mr-5 shadow-md">
            <div className="flex flex-row items-center justify-between px-4">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-primaryColor text-white rounded-lg hover:bg-opacity-90 transition-colors"
              >
                Back to List
              </button>
            </div>
            ...
            <>
              ...
              {currentProducts.map(([productName, products]) => (
                ...
                >
                  ...
                  <ProductItemTable products={products} addToCart={addToCart} />
                </div>
              ))}
...
export default AddProductOnEdit;
