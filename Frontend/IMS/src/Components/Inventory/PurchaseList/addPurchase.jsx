...
const AddPurchase = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        products.forEach((product, productIndex) => {
            ...
            formData.append(`products[${productIndex}][expire_date]`, product.expire_date);
            formData.append(`products[${productIndex}][purchase_date]`, product.purchase_date);
            formData.append(`products[${productIndex}][serial_number]`, product.serial_number);
            formData.append(`products[${productIndex}][batch_number]`, product.batch_number);
            formData.append(`products[${productIndex}][purchase_price]`, product.itemCost);
            formData.append(`products[${productIndex}][additional_cost]`, product.additionalCost);
            formData.append(`products[${productIndex}][overall_cost]`, product.totalCost);
            formData.append(`products[${productIndex}][selling_price]`, product.sellingPrice);
        ...
        payments.forEach((payment, paymentIndex) => {
            formData.append(`payments[${paymentIndex}][payment_type]`, payment.payment_type);
            formData.append(`payments[${paymentIndex}][payment_option]`, payment.payment_option);
            formData.append(`payments[${paymentIndex}][pre_notification_day]`, payment.pre_notification_day);
            formData.append(`payments[${paymentIndex}][bank_name]`, payment.bank_name);
            formData.append(`payments[${paymentIndex}][account_number]`, payment.account_number);
        ...
        <div className="flex-1 p-4">
            <div>
                <div className="flex flex-row justify-between items-center">
                    ...
                    <button onClick={() => setAddProductOpen(true)} className="primaryBtn">+ Add Product</button>
                </div>
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-800">
                            <tr>
                                ...
                                <th className="p-4 text-left text-white font-semibold">Purch. Price</th>
                                ...
                                <th className="p-4 text-left text-white font-semibold">Total</th>
                                <th className="p-4 text-left text-white font-semibold">Price</th>
                                <th className="p-4 text-left text-white font-semibold">Expire Date</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {products.map((product, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="p-4 text-gray-700">{productName(product.item_id)}</td>
                                    <td className="p-4 text-gray-700">{FormattedNumber(product.itemCost)}</td>
                                    <td className="p-4 text-gray-700">{product.unit}</td>
                                    <td className="p-4 text-gray-700">{FormattedNumber(product.quantity)}</td>
                                    <td className="p-4 text-gray-700">{FormattedNumber(product.quantity * product.itemCost)}</td>
                                    <td className="p-4 text-gray-700">{FormattedNumber(product.sellingPrice)}</td>

                                    <td className="p-4 text-gray-700">{FormattedDate(product.expire_date)}</td>
                        ...
                        <tfoot>
                            <tr className="bg-gray-200 font-semibold">
                                ...
                                <td className="p-4 text-gray-800"></td>
                                <td className="p-4 text-gray-800" colSpan={2}> Total</td>
                                <td className="p-4 text-gray-800">{FormattedNumber(products.reduce((sum, product) => sum + parseFloat(product.quantity), 0))}</td>
                                <td className="p-4 text-gray-800  " colSpan={2}>
                                    <div className="flex flex-row gap-1">
                                        <p> {FormattedNumber(
                                            products.reduce((sum, product) => sum + parseFloat(product.quantity) * parseFloat(product.itemCost), 0)
                                        )}</p>
                                        <p>ETB</p>
                                    </div>
                                </td>
            ...
            <div className="mt-8">
                <div className="flex flex-row justify-between items-center">
                    ...
                    <button onClick={() => setAddPaymentOpen(true)} className="primaryBtn">+ Add Payment</button>
                </div>
                <div className="overflow-hidden rounded-lg shadow-md border border-gray-200">
                    <table className="w-full">
                        <thead className="bg-gray-800">
                            <tr>
                                <th className="p-4 text-left text-white font-semibold">No.</th>
                                <th className="p-4 text-left text-white font-semibold">Payment Type</th>
                                <th className="p-4 text-left text-white font-semibold">Pay By</th>
                                <th className="p-4 text-left text-white font-semibold">Amount</th>
                                <th className="p-4 text-left text-white font-semibold">Remark</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {payments.map((payment, index) => (
                                <tr key={index}>
                                    ...
                                    <td className="p-4 text-gray-700">{payment.payment_option}</td>
                                    <td className="p-4 text-gray-700">{payment.amount}</td>
                                    <td className="p-4 text-gray-700">{payment.remark}</td>
                                    <td className="p-4 text-gray-700">{FormattedDate(payment.payment_date)}</td>
                                    <td className="p-4 relative">
                                        <button onClick={() => handlePaymentMenuToggle(index)} className="text-gray-500 hover:text-gray-700 ">
                                            <FaEllipsisV />
                                        </button>
            ...
            <div className="bg-white my-4 rounded-lg shadow-md grid grid-cols-1 md:grid-cols-2 gap-4 p-6">
                ...
                <div className="space-y-2 border p-2 rounded-lg">
                    <div className="flex flex-row gap-1">
                        ...
                        <p className="font-semibold"> {FormattedNumber(
                            products.reduce((sum, product) => sum + parseFloat(product.quantity) * parseFloat(product.itemCost), 0)
                        )}</p>
                    ...
                    <div className="flex flex-row gap-1">
                        ...
                        <p className="font-semibold" >{payments.reduce((sum, payment) => {
            ...
            {/* Modals */}
            <AddProduct isOpen={isAddProductOpen} close={() => setAddProductOpen(false)} addProduct={addProduct} />
            <AddPayment isOpen={isAddPaymentOpen} close={() => setAddPaymentOpen(false)} addPayment={addPayment} />
            <ProductDetail
