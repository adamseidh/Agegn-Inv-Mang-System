...
export const TotalProductsCard = (props) => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      try {
        const { data } = await dataProvider.getList("ClientStock", {
...
export const TotalSuppliersCard = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await dataProvider.getList("supplierCount", {
          sort: { field: "id", order: "ASC" },
          filter: {},
        });
...
export const TotalCustomersCard = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await dataProvider.getList("customersCount", {
          sort: { field: "id", order: "ASC" },
          filter: {},
        });
