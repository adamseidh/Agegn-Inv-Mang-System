...
const singleItem = (req, res) => {
    const { id } = req.params;
    const query = `SELECT i.*, c.name as categoryName
         FROM items i
        JOIN category c
        ON i.category_id = c.id WHERE i.id = ?`;
...
const EditItem = (req, res) => {
    ...
    db.query(query, [name, category_id, id], (err, result) => {
        if (err) {
            return res.status(500).json({ error: err });
        }
...
module.exports = { Items, singleItem, AddItem, EditItem, deleteITem }