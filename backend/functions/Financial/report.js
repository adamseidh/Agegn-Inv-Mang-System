...
const purchasedProducts = (req, res) => {
  ...
  const query = `
      SELECT pl.*, i.name as productName,pp.payment_status, pp.amount as paymentAmount  FROM product_list pl
      LEFT JOIN items i ON pl.item_id = i.id
      LEFT JOIN purchase_list pchL ON pl.purchase_id = pchL.id
      LEFT JOIN purchase_payment pp ON pchL.id = pp.purchase_id 
    `;
    ...
    if (err) {
      ...
      db.query(countQuery, (err, countResult) => {
        if (err) {
          ...
          res.setHeader(
            "Content-Range",
            `product_list ${offset}-${offset + results.length}/${total}`
          );
