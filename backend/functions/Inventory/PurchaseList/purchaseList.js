...
const PurchaseList = (req, res) => {
    ...
    const query = `
        SELECT PL.*, I.name  FROM purchase_list PL 
        LEFT JOIN items I 
        ON PL.product_id = I.id
        ...
        if (err) {
            ...
            db.query(countQuery, (err, countResult) => {
                if (err) {
                    ...
                    res.setHeader('Content-Range', `purchase_list ${offset}-${offset + results.length}/${total}`);
...
const CreateItem = (req, res) => {
    const { name, low_level, category_id, type_id, description, serverHost } = req.body;
    ...
    const values = [name, low_level, category_id || '', type_id || '', description || '', fullPath];
...
const EditItem = (req, res) => {
    ...
    const { name, low_level, category_id, type_id, description, serverHost, oldImage } = req.body;
    const image = req.file ? req.file.filename : null;
    const { id } = req.params;
    ...
    db.query(query, [name, low_level, category_id, type_id, description, fullPath, id], (err, result) => {
        if (err) {
            return res.status(500).json({ error: err });
        }
...
module.exports = { PurchaseList, AnItem, CreateItem, EditItem, deleteItem }
