...
const AddSupplier = (req, res) => {
    const {
        ...
        phone,
        email,
        website, region, wereda_or_city, kebele, tin, letter_no
...
const EditSupplier = (req, res) => {
    ...
    db.query(query, [name, phone, email, website, region, wereda_or_city, kebele, tin || '', letter_no || '', id], (err, result) => {
        if (err) {
            return res.status(500).json({ error: err });
        }
...
module.exports = { Supplier, singleSupplier, AddSupplier, EditSupplier, deleteSupplier }