...
const SalesUpcamingPayments = (req, res) => {
  const query = `
    SELECT 
      p.payment_date,
    ...
    WHERE 
      ...
      AND DATE(p.payment_date) <= DATE_ADD(CURDATE(), INTERVAL p.pre_notification_day DAY)
...
const checkSalesUpcamingPayments = async (req, res) => {
  try {
    ...
    LEFT JOIN customers c ON sl.customer_id = c.id
    WHERE 
      DATE(p.payment_date) = DATE_ADD(CURDATE(), INTERVAL p.pre_notification_day DAY)
      AND p.payment_status = 'Not Completed'
  `;
    ...
    db.query(query, async (error, payments, fields) => {
      ...
      if (payments && payments.length > 0) {
        ...
        for (const payment of payments) {
          ...
          const notificationDescription = `Payment of ${payment.amount} ETB from customer ${payment.customerName} is after ${payment.pre_notification_day} day, on (${payment.payment_date}).`;
