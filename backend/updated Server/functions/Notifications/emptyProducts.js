...
const EmptyProducts = (req, res) => {
  ...
  // Base query wrapped in a subquery to allow filtering by calculated alias
  let baseQuery = `
  SELECT 
  p.id,
    i.id AS item_id,
    ...
    COALESCE((
      ...
      INNER JOIN product_list p2 ON s.product_id = p2.id
...
`;
  ...
  db.query(baseQuery, (err, results) => {
    if (err) {
      ...
      const finalResults = Object.values(mergedProducts).map((item) => ({
        id: item.id,
      ...
      db.query(countQuery, (err, countResult) => {
        if (err) {
          ...
          res.setHeader(
            "Content-Range",
            `emptyProducts ${offset}-${offset + finalResults.length}/${total}`
          );
...
const EmptyProductsCount = (req, res) => {
    ...
    if (err) {
      ...
      db.query(countQuery, (err, countResult) => {
        if (err) {
          ...
          // Set headers and return the response
          res.setHeader("Content-Range", `stock `);
          res.setHeader("Access-Control-Expose-Headers", "Content-Range");
          res.json(finalResults);
        }
      });
